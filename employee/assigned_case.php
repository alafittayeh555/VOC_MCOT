...
$sql = "SELECT c.*, u.full_name as complainer 
        ...
        AND c.status NOT IN ('Processed', 'Resolved', 'Completed', 'Cancelled')";
...
// Order by priority (Pending > Rejected > Review) then date
$sql .= " ORDER BY FIELD(c.status, 'Review', 'Rejected', 'Pending') DESC, c.created_at ASC";
...
<div class="head-title">
    <div class="left">
        <h1><?php echo __('menu_assigned_cases', 'Assigned Cases'); ?></h1>
        <ul class="breadcrumb">
            <li><a href="dashboard.php"><?php echo __('menu_dashboard', 'Dashboard'); ?></a></li>
            <li><i class='bx bx-chevron-right'></i></li>
            <li><a class="active" href="#"><?php echo __('menu_assigned_cases', 'Assigned Cases'); ?></a></li>
        </ul>
    </div>
</div>
        ...
        <div class="assigned-toolbar">
            ...
            <form method="GET" id="filterForm" style="display: flex; gap: 10px; align-items: center; flex-wrap: nowrap;">
                ...
                <?php if($status || $search): ?>
                    <a href="assigned_case.php" class="reset-btn" title="<?php echo __('assigned_btn_reset', 'Reset'); ?>">
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width: 15%; text-align: center;"><?php echo __('assigned_th_due_date', 'Due Date'); ?></th>
                    ...
                    <?php if (count($complaints) > 0): ?>
                        <?php foreach ($complaints as $index => $complaint): ?>
                            <tr>
                                ...
                                <td style="text-align: center; color: var(--dark); font-weight: 400;">
                                    <?php 
                                    if (!empty($complaint['due_date'])) {
                                        $due = strtotime($complaint['due_date']);
                                        $today = strtotime(date('Y-m-d'));
                                        $color = ($due < $today && $complaint['status'] == 'In Progress') ? 'var(--red)' : 'var(--dark)';
                                        echo '<span style="color: '.$color.'; font-weight: 500;">' . date('d/m/Y', $due) . '</span>';
                                    } else {
                                        echo '-';
                                    }
                                    ?>
                                ...
                                <td style="text-align: center;">
                                    <a href="case_details.php?id=<?php echo $complaint['id']; ?>" style="color: var(--dark-grey); font-size: 20px; margin: 0 5px;" title="View Details">
                                        <i class='bx bx-show'></i>
                                    </a>
                                </td>
                            </tr>
                        ...
                        <tr>
                            <td colspan="5" style="padding: 50px;">
                                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%;">
                                    ...
                                    <p style="color: var(--dark-grey); font-size: 16px; margin: 0; white-space: nowrap;"><?php echo __('assigned_empty', 'No assigned complaints found'); ?></p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
