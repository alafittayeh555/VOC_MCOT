...
foreach ($all_attachments as $att) {
    // Logic: If uploaded > 5 mins after complaint creation, assume it's from staff/resolution
    $att_ts = strtotime($att['uploaded_at']);
    if ($att_ts > ($comp_ts + 300)) {
        $employee_attachments[] = $att;
    } else {
...
<div class="history-container">
    <div class="head-title">
        ...
        <a href="history.php" class="btn-download">
    ...
    <div class="table-data">
        ...
        <div class="order" style="width: 100%;">
            ...
            <!-- Description -->
            <div style="background: var(--light); border: 1px solid var(--grey); border-radius: 12px; padding: 25px; margin-bottom: 20px;">
            ...
            <!-- Employee Note / Resolution -->
            <?php if (!empty($complaint['employee_note'])): ?>
                <div style="background: #ecfdf5; border: 1px solid #10b981; border-radius: 12px; padding: 25px; margin-bottom: 30px; position: relative; overflow: hidden;">
                    <div style="position: absolute; right: -15px; top: -15px; font-size: 80px; opacity: 0.1; color: #10b981; transform: rotate(15deg);">
                        <i class='bx bxs-quote-right'></i>
                    </div>
                    <h4 style="font-weight: 700; margin-bottom: 15px; display: flex; align-items: center; gap: 10px; color: #064e3b;">
                        <i class='bx bxs-check-shield' style="font-size: 24px;"></i> 
                        <?php echo __('pr_dept_note', 'บันทึกการแก้ไขจากหน่วยงาน'); ?>
                    </h4>
                    <div class="whitespace-pre-wrap" style="color: #064e3b; font-size: 15px; line-height: 1.7; font-weight: 500;">
                        <?php echo trim($complaint['employee_note']); ?>
                    </div>
                </div>
            <?php endif; ?>
             ...
             <?php if (count($employee_attachments) > 0): ?>
                <div style="margin-top: 30px; padding-top: 20px; border-top: 1px dashed var(--grey);">
                    <h4 style="font-weight: 600; margin-bottom: 15px; display: flex; align-items: center; gap: 10px; color: var(--dark);">
                        <i class='bx bx-briefcase-alt-2' style="color: var(--green);"></i> 
                        <?php echo __('pr_dept_attachments', 'ไฟล์แนบจากหน่วยงาน'); ?> (<?php echo count($employee_attachments); ?>)
                    </h4>
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 15px;">
                        <?php foreach ($employee_attachments as $att): ?>
                            <?php 
                            $fileExt = strtolower(pathinfo($att['file_path'], PATHINFO_EXTENSION));
                            $isImage = in_array($fileExt, ['jpg', 'jpeg', 'png', 'gif']);
                            $filePath = $att['file_path'];
                            if (strpos($filePath, 'assets/') === false) { $filePath = 'assets/uploads/complaints-file/' . $filePath; }
                            ?>
                            <a href="../<?php echo $filePath; ?>" target="_blank" 
                               style="display: block; background: #ecfdf5; border: 1px solid #10b981; border-radius: 10px; overflow: hidden; text-decoration: none;">
                                <div style="height: 100px; background: #f0fdf4; display: flex; align-items: center; justify-content: center; overflow: hidden;">
                                    <?php if ($isImage): ?>
                                ...
                                <div style="padding: 10px;">
                                    <p style="font-size: 12px; font-weight: 600; color: #064e3b; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 2px;">
                                        <?php echo htmlspecialchars($att['file_name'] ?? 'File'); ?>
                                    </p>
                                    <p style="font-size: 10px; color: #10b981;"><?php echo strtoupper($fileExt); ?></p>
                                </div>
                            </a>
                        <?php endforeach; ?>
                    </div>
